...
module.exports = function() {
	...
	const router = function(req, res, next) {
		// Parse url path and assign properties to req
		...
		/*
		Assign empty object to req.params (alignment with http requests
		and prevents errors due to undefined property)
		*/
		req.params = {};
				...
				if (handler) {
					if (handler.method === "all" || handler.method === method) {
						...
						if (handler.callback.isRouter) {
							if (req.path.startsWith(handlerPath)) {
								req.handled = true;
								result = handler;
								break;
							}
						...
						const matches = re.exec(req.path);
